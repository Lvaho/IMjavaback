...
import zjc.zhixun.admin.service.LogHelper;
import zjc.zhixun.core.util.RegexUtil;
...
import static zjc.zhixun.admin.util.AdminResponseCode.*;
...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<ZhiXunAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(ZhiXunAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody ZhiXunAdmin admin) {
        ...
        List<ZhiXunAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        ZhiXunAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody ZhiXunAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody ZhiXunAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        ZhiXunAdmin currentAdmin = (ZhiXunAdmin) currentUser.getPrincipal();
